...
public:
  ...
  inline bool contain(uint64_t key) const {
    ...
    uint32_t r1 = (uint32_t)xor_rotl64(hash, 21);
    uint32_t r2 = (uint32_t)xor_rotl64(hash, 42);
    uint32_t h0 = xor_reduce(r0, m_blockLength);
    uint32_t h1 = xor_reduce(r1, m_blockLength) + m_blockLength;
    uint32_t h2 = xor_reduce(r2, m_blockLength) + 2 * m_blockLength;
  ...
  bool populateBuffered(const uint64_t *keys, uint32_t size) {
    ...
    while (true) {
      ...
      for (size_t i = 0; i < size; i++) {
        uint64_t key = keys[i];
        xor_hashes_t hs = get_h0_h1_h2(key);
        xor_buffered_increment_counter(hs.h0, hs.h, &buffer0, sets0);
        xor_buffered_increment_counter(hs.h1, hs.h, &buffer1,
                                      sets1);
        xor_buffered_increment_counter(hs.h2, hs.h, &buffer2,
      ...
      while (Q0size + Q1size + Q2size > 0) {
        while (Q0size > 0) {
          ...
          //sets0[index].count = 0;
          uint64_t hash = keyindex.hash;
          uint32_t h1 = get_h1(hash);
          uint32_t h2 = get_h2(hash);
    ...
    ItemType * fingerprints0 = m_fingerprints;
    ItemType * fingerprints1 = m_fingerprints + blockLength;
    ItemType * fingerprints2 = m_fingerprints + 2 * blockLength;
    ...
    while (stack_size > 0) {
      ...
      uint64_t val = xor_fingerprint(ki.hash);
      if(ki.index < blockLength) {
        val ^= fingerprints1[get_h1(ki.hash)] ^ fingerprints2[get_h2(ki.hash)];
      } else if(ki.index < 2 * blockLength) {
        val ^= fingerprints0[get_h0(ki.hash)] ^ fingerprints2[get_h2(ki.hash)];
      } else {
        val ^= fingerprints0[get_h0(ki.hash)] ^ fingerprints1[get_h1(ki.hash)];
      }
      m_fingerprints[ki.index] = val;
    }
    xor_free_buffer(&buffer0);
    xor_free_buffer(&buffer1);
    xor_free_buffer(&buffer2);
...
private:
  inline xor_hashes_t get_h0_h1_h2(uint64_t k) {
    uint64_t hash = xor_mix_split(k, m_seed);
    xor_hashes_t answer;
    answer.h = hash;
    uint32_t r0 = (uint32_t)hash;
    uint32_t r1 = (uint32_t)xor_rotl64(hash, 21);
    ...
    answer.h0 = xor_reduce(r0, m_blockLength);
    answer.h1 = xor_reduce(r1, m_blockLength);
    answer.h2 = xor_reduce(r2, m_blockLength);
    return answer;
  }

  inline uint32_t get_h0(uint64_t hash) {
    uint32_t r0 = (uint32_t)hash;
    return xor_reduce(r0, m_blockLength);
  }

  inline uint32_t get_h1(uint64_t hash) {
    uint32_t r1 = (uint32_t)xor_rotl64(hash, 21);
    return xor_reduce(r1, m_blockLength);
  }

  inline uint32_t get_h2(uint64_t hash) {
    uint32_t r2 = (uint32_t)xor_rotl64(hash, 42);
    return xor_reduce(r2, m_blockLength);
  }
